...
- **자가 치유 지능 / Auto-Healing Intelligence**: 추론 시 가중치 누락을 감지하고 자동으로 학습-추론 체인을 가동하는 자가 복구 로직 및 GUI/CLI 드롭다운 선택기 구현 완료. / Implemented auto-healing logic (auto-train on missing weights) and dynamic model selection UI for GUI/CLI.
- **오디세이 최종 진화 / Final Odyssey Evolution**: 한영 병기 및 코딩 지원, KV 캐싱, 이중 인터페이스 추론 기능을 포함한 v1.0.0 정식 릴리스. / Official release of v1.0.0 including KR/EN/Code support, KV Caching, and dual-interface inference.
